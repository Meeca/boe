...
//                [imgView setImageWithURL:[NSURL URLWithString:imgURL]];
                [imgView sd_setImageWithURL:[NSURL URLWithString:imgURL] placeholderImage:KZHANWEI];
            }
            else
            {
...
//        B-16-2
        //说明文字层
        UIImageView *noteView=[[UIImageView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-44,self.bounds.size.width,44)];
        ...
        pageControl=[[UIPageControl alloc]initWithFrame:CGRectMake((self.frame.size.width-pageControlWidth),12, pageControlWidth, pagecontrolHeight)];
        pageControl.currentPage=0;
        pageControl.numberOfPages=(pageCount-2);
        [noteView addSubview:pageControl];
        
        noteTitle=[[UILabel alloc] initWithFrame:CGRectMake(15, 0, self.frame.size.width-pageControlWidth-25, 44)];
        [noteTitle setText:[titleArray objectAtIndex:0]];
        [noteTitle setBackgroundColor:[UIColor clearColor]];
        [noteTitle setFont:[UIFont boldSystemFontOfSize:18]];
