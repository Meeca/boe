...
//        [button setFrame:CGRectMake(xOffset, 0, textSize.width, kHeightOfTopScrollView)];
        [button setFrame:CGRectMake((KSCREENWIDTH-180)/3+((KSCREENWIDTH-180)/3+90)*i, 0, 90, kHeightOfTopScrollView)];
//        button.backgroundColor = [UIColor grayColor];
        //计算下一个tab的x偏移量
        xOffset += textSize.width + kWidthOfButtonMargin;
        [button setTag:i+100];
        if (i == 0) {
            self.shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, button.height-2, button.width+20, 2);
