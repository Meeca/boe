...
- (void)loadCache
{
    self.recommends = [ArtistInfo readFromUserDefaults:@"ArtistModel.recommends"];
}

- (void)saveCache
{
    [ArtistInfo userDefaultsWrite:[self.recommends objectToString] forKey:@"ArtistModel.recommends"];
}

- (void)clearCache
{
    [ArtistInfo userDefaultsRemove:@"ArtistModel.recommends"];
}
...
{
    [API_APP_PHP_FINDS_ARTIST cancel];
    
    API_APP_PHP_FINDS_ARTIST * api = [API_APP_PHP_FINDS_ARTIST api];
    ...
    api.whenUpdate = ^
    {
        ...
        else
        {
            ...
            NSLog(@"----API_APP_PHP_FINDS_ARTIST---");
