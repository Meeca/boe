...
-(void)updateDirection:(MessageType)direction{
    ...
    NSString *bubbleNamed = direction == MessageType_RECEIVE ? @"chat_from_bg_normal" : @"chat_to_bg_normal";
    self.bubbleImageView.image = [[UIImage imageNamed:bubbleNamed] stretchableImageWithLeftCapWidth:15 topCapHeight:25];
    
    [self.avatarImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        if (direction == MessageType_RECEIVE) {
            SMAS(make.left.equalTo(self.contentView.mas_left).offset(10));
        }else{
...
-(void)updateMessage:(MessageModel *)message showDate:(BOOL)showDate{
    ...
    [_avatarImageView sd_setImageWithURL:[NSURL URLWithString:message.image] placeholderImage:[UIImage imageNamed:@"chatfrom_doctor_icon"]];
    
    [self updateDirection:message.messageType];
    [self updateDate:message.created_at showDate:showDate];
    self.message = message;
    self.msgStatus = message.messageStatus;
}
-(void)setMsgStatus:(MessageStatus)msgStatus{
    if (msgStatus == MessageType_SEND) {
        self.retryBtn.hidden = YES;
        self.activityIndicator.hidden = YES;
    }
    self.retryBtn.hidden = msgStatus != MessageStatus_FAILED;
    if (msgStatus == MessageStatus_SENDING) {
